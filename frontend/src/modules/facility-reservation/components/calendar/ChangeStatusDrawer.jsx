...
export default function ChangeStatusDrawer({ selectedDate }) {
  ...
  const handleToggle = async (makeAvailable) => {
    if (!makeAvailable && !reason.trim()) {
      notify.error('You must provide a reason when making slots unavailable.');
      return;
    }
    
    if (!makeAvailable && reason.length > 150) {
      notify.error('Reason cannot exceed 150 characters.');
      return;
    }
    ...
    try {
      ...
      notify.success(`Successfully marked ${slotsToManage.length} slot(s) as ${makeAvailable ? 'available' : 'unavailable'}.`);
      setReason('');
      triggerRefresh();
      closeDrawer();
      ...
      notify.error(err.response?.data?.message || 'Failed to toggle slots');
    } finally {
