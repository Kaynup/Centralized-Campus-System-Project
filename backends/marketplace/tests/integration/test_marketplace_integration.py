...
class TestMarketplaceIntegration:
    ...
    def test_create_item(self, client, test_seller):
        # Mock auth to simulate test_seller
        app.dependency_overrides[get_current_user] = mock_get_current_user(test_seller)
        ...
        resp = client.post(f"/marketplace/items/?seller_id={test_seller['id']}", json=payload)
    ...
    def test_create_item_auth_mismatch(self, client, test_seller, test_buyer):
        # Attempt to create an item as a different user
        app.dependency_overrides[get_current_user] = mock_get_current_user(test_buyer)
    ...
    def test_purchase_item_flow(self, client, test_seller, test_buyer):
        ...
        purchase_resp = client.post("/marketplace/transactions/purchase", json=purchase_payload)
        ...
        purchase_resp2 = client.post("/marketplace/transactions/purchase", json=purchase_payload)
        assert purchase_resp2.status_code == 409

        # 4. Confirm delivery as buyer
        confirm_resp = client.post(f"/marketplace/delivery/confirm/{holding_id}")
        assert confirm_resp.status_code == 200
        assert float(confirm_resp.json()["amount_released"]) == 30.00

        app.dependency_overrides.clear()

    def test_get_items(self, client, test_seller):
        app.dependency_overrides[get_current_user] = mock_get_current_user(test_seller)
    ...
    def test_purchase_own_item_fails(self, client, test_seller):
        app.dependency_overrides[get_current_user] = mock_get_current_user(test_seller)
    ...
    def test_purchase_insufficient_balance(self, client, test_seller, test_buyer):
        app.dependency_overrides[get_current_user] = mock_get_current_user(test_seller)
