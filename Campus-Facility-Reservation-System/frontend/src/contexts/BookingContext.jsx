...
export function BookingProvider({ children }) {
  ...
  const toggleSlotSelection = (slot, facility) => {
    if (!slot || (slot.status && slot.status !== 'AVAILABLE')) {
      showWarning('Only green available slots can be selected.');
      return;
    }

    if (selectedSlots.length > 0 && facility?.id !== multiSelectFacility?.id) {
      showWarning('Please select slots from the same facility only.');
      return;
    }
    ...
    if (!areSlotsConsecutive(nextSelection)) {
      showWarning('Selected slots must be adjacent and without gaps.');
      return;
    }
  ...
  const reserveSelectedSlots = () => {
    if (selectedSlots.length === 0) {
      showWarning('Select available slots before reserving.');
      return;
    }
    if (!multiSelectFacility) {
      showWarning('Please select slots from the same facility before reserving.');
      return;
    }
  ...
  const changeStatusSelectedSlots = () => {
    if (selectedSlots.length === 0) {
      showWarning('Select slots before changing status.');
      return;
    }
    if (!multiSelectFacility) {
      showWarning('Please select slots from the same facility.');
      return;
    }
    setSelectedSlot(null);
  ...
  const confirmMultiSelectBooking = async (selectedDateStr) => {
    ...
    try {
      ...
      if (selectedFacility?.requiresApproval) {
        showInfo(`Booking request submitted. Awaiting approval.`);
      } else {
        showSuccess(`Booking confirmed for ${facilityName}!`);
      }
    } catch (err) {
  ...
  const confirmCancellation = async (bookingId, reason) => {
    ...
    try {
      ...
      if (penalty > 0) {
        showWarning(`Booking cancelled. You forfeited ${penalty} tokens.`);
      } else {
        showSuccess(`Booking cancelled. ${refund} tokens refunded.`);
      }
    } catch (err) {
      console.error('Cancellation failed:', err);
      setBookingError(err.response?.data?.message || err.message || 'Cancellation failed');
      showError(`Cancellation failed: ${err.response?.data?.message || err.message || 'Something went wrong.'}`);
      throw err;
    } finally {
      setIsBookingInProgress(false);
    }
