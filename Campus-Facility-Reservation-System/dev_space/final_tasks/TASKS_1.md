...
> **Owns**: Project scaffolding, database schema, migrations, auth system, local environment setup, seed scripts, token system foundation
...
## Status (synced from dev-docs/Whats_left_of_task1.md)

- **Completed (confirmed):** root repo structure and top-level folders; `README.md` with backend setup; backend package scaffolding under `backend/app/`; `backend/app/core/config.py`; `backend/app/core/logging.py`; DB files `backend/app/db/models.py`, `schemas.py`, `session.py` and migration scaffolding; basic auth helpers in `backend/app/api/v1/auth.py` and `backend/app/core/security.py`; test harness fixture at `backend/app/tests/conftest.py` (tests run); `scripts/setup_db.sh` and `scripts/seed_db.py`; `docs/setup.md` and `docs/architecture.md`; Makefile commands for backend.

- **Remaining (from dev-docs):** incomplete `backend/app/db/crud.py` review (most CRUD implemented), health endpoint needs DB readiness checks, `app/core/security.py` contains TODOs (get_current_user implemented), `docs/api_spec.md` is still a placeholder.
- **Work completed (implemented in code):** utilities (`time_utils`, `role_helpers`, `exceptions`), services (`user_service`, `facility_service`, `booking_service`, `approval_service`, `cancellation_service`), API routers (`bookings.py`, `approvals.py`, `facilities.py`, `tokens.py`).


You are responsible for everything that makes the project runnable and data-coherent. Without your work, no other team member can proceed past local setup. Your deliverables are the bedrock: a local environment, a versioned schema, a working auth system, and a clean data access layer.
...
- [ ] Define all required environment variables:
  ...
  - [ ] `DATABASE_URL: str` — e.g. `mysql+pymysql://user:pass@localhost:3306/campus_db`
...
## Phase 12 — Local Environment Setup

### 12.1 Python Virtual Environment
...
- [ ] From the `backend/` directory, create and activate a virtual environment:
  ...
  pip install -r requirements.txt
  ```
- [ ] Copy `.env.example` to `.env` and fill in real values for local dev
- [ ] Confirm `venv/` is listed in root `.gitignore` — never commit it

### 12.2 MySQL 8.0 Local Setup

- [ ] Install MySQL 8.0 directly on your machine (via Homebrew, apt, or the MySQL installer)
...
- [ ] Log in as root and create the database and user:
  ...
  FLUSH PRIVILEGES;
  ```
- [ ] Set `DATABASE_URL=mysql+pymysql://campus_user:yourpassword@localhost:3306/campus_db` in `backend/.env`
- [ ] Verify connection: run `alembic upgrade head` and confirm tables appear with `SHOW TABLES;`
...
- [ ] Add a comment block explaining this is run once on first-time local setup

### 12.4 Running the Backend Manually
...
- [ ] Start the server in development mode (with auto-reload):
  ...
  ```
- [ ] For production-style local testing (no reload):
  ```bash
...
- [ ] MySQL 8.0 is running locally and `alembic upgrade head` succeeds with no errors
...
- [ ] `README.md` documents how to start the project from scratch (venv + MySQL + uvicorn)
